...
public:
	...
	edge(vertex* a, vertex* b,string s) {
		this->source = a;
		this->s = s;
		this->destination = b;
...
void DFS(vertex* V) {
	if (V->incidentEdgeList->size == 0)	//예외처리 보완
		return;
	cout << V->data << " ";
	V->visited = true;
...
int main() {
	...
	cout << endl;
	edge* ite = g->TotaledgeList->head;
	vector<string> Back;
	while (1) {
		if (ite->discovery == true)
			cout << ite->s << " ";
		else if (ite->back == true)
			Back.push_back(ite->s);
		if (ite != g->TotaledgeList->tail)
			ite = ite->next;
		else break;
	}
	cout << endl;
